...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    const r = await axios.post(`${serverBase}/auth/google/signUp`, req.body, { //req.body: { name, age, status }
      headers: {
    ...
    if (setCookie) res.setHeader("Set-Cookie", setCookie); // 서버 응답 헤더에 Set-Cookie(백엔드가 준 쿠키)를 넣어라
